...
- inversion counting :: [트리 > 세그먼트 트리 >] 세그먼트 트리를 이용해 선분의 교차점 개수를 구하는 알고리즘
- bottom-up :: [다이나믹 프로그래밍 >] 상향식 DP, 작은 문제들부터 풀어나가며 전체 문제의 답을 찾는 DP 방식, 재귀 호출을 일반적으로 사용하지 않음
- top-down :: [다이나믹 프로그래밍 >] 메모제이션, 하향식 DP, 큰 문제를 해결하기 위해 작은 문제들을 호출하는 방식
